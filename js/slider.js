...
(function () {
  ...
  function handleCredibilitySliderState() {
    ...
    // 1) Initialize sliders that have explicit configs
    Object.entries(configMap).forEach(([selector, opts]) => {
      document.querySelectorAll(selector).forEach((el) => {
          // Pass the specific options for this selector
          initSingleSlider(el, opts); 
      });
    });

    // 2) Initialize any remaining .swiper elements with defaults
    document.querySelectorAll('.swiper').forEach((el) => {
      // Skip if already initialized via configMap or specific logic
      if (el.swiper || el.classList.contains('credibility-swiper-mobile')) return;
      initSingleSlider(el); // Initialize with DEFAULTS
    });

    // 3) Initial check for mobile slider state
    handleCredibilitySliderState();

    // 4) Add debounced resize listener for mobile slider state
