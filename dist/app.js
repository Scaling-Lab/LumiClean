...
(function() {
  const configMap = {
    ...
    ".credibility-swiper-mobile": {
      init: false,
      // Initialize manually based on screen size
      loop: true,
      autoplay: {
        delay: 3500,
        // Adjust delay as needed (e.g., 3500ms)
        disableOnInteraction: false,
        // Keep playing after user interacts
        pauseOnMouseEnter: true
        // Pause when mouse hovers (good practice, though less relevant for pure mobile)
      }
  ...
  function autoDetectControls(container, opts) {
    ...
    if (!opts.navigation) {
      const prev = container.querySelector(".swiper-button-prev, .swiper-prev, .testimonial-prev, .product-prev") || container.parentElement?.querySelector(
        ".swiper-button-prev, .swiper-prev, .testimonial-prev, .product-prev"
      );
      const next = container.querySelector(".swiper-button-next, .swiper-next, .testimonial-next, .product-next") || container.parentElement?.querySelector(
  ...
  function initSingleSlider(container, customOpts = {}) {
    const selector = container.classList.contains("credibility-swiper-mobile") ? ".credibility-swiper-mobile" : container.classList.contains("product-slider") ? ".product-slider" : container.classList.contains("testimonial-swiper") ? ".testimonial-swiper" : null;
    ...
    if (opts.init !== false) {
      ...
      swiper.on("init", () => {
        attachHoverHandlers(container, swiper, swiper.params);
      });
    }
    return swiper;
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    Object.entries(configMap).forEach(([selector, opts]) => {
      document.querySelectorAll(selector).forEach((el) => {
        initSingleSlider(el, opts);
      });
    });
    document.querySelectorAll(".swiper").forEach((el) => {
      if (el.swiper || el.classList.contains("credibility-swiper-mobile")) return;
      initSingleSlider(el);
    });
    handleCredibilitySliderState();
    window.addEventListener("resize", debounce(handleCredibilitySliderState, 250));
  });
})();
...
fragment CartFragment on Cart {
  ...
  lines(first: 50) {
    nodes {
      ...
      merchandise { ... on ProductVariant { id title price { amount currencyCode } compareAtPrice { amount currencyCode } image { url altText } product { title } } }
    }
  }
...
var CREATE_CART_MUTATION = `mutation cartCreate($cartInput: CartInput!) { cartCreate(input: $cartInput) { cart { ...CartFragment } userErrors { field message } } } ${CartFragment}`;
var FETCH_CART_QUERY = `query getCart($cartId: ID!) { cart(id: $cartId) { ...CartFragment } } ${CartFragment}`;
var ADD_TO_CART_MUTATION = `mutation cartLinesAdd($cartId: ID!, $lines: [CartLineInput!]!) { cartLinesAdd(cartId: $cartId, lines: $lines) { cart { ...CartFragment } userErrors { field message } } } ${CartFragment}`;
var REMOVE_FROM_CART_MUTATION = `mutation cartLinesRemove($cartId: ID!, $lineIds: [ID!]!) { cartLinesRemove(cartId: $cartId, lineIds: $lineIds) { cart { ...CartFragment } userErrors { field message } } } ${CartFragment}`;
var UPDATE_CART_LINE_MUTATION = `mutation cartLinesUpdate($cartId: ID!, $lines: [CartLineUpdateInput!]!) { cartLinesUpdate(cartId: $cartId, lines: $lines) { cart { ...CartFragment } userErrors { field message } } } ${CartFragment}`;
var UPDATE_DISCOUNT_CODE_MUTATION = `mutation cartDiscountCodesUpdate($cartId: ID!, $discountCodes: [String!]) { cartDiscountCodesUpdate(cartId: $cartId, discountCodes: $discountCodes) { cart { ...CartFragment } userErrors { field message code } } } ${CartFragment}`;
var CART_ATTRIBUTES_UPDATE_MUTATION = `mutation cartAttributesUpdate($cartId: ID!, $attributes: [AttributeInput!]!) { cartAttributesUpdate(cartId: $cartId, attributes: $attributes) { cart { ...CartFragment } userErrors { field message } } } ${CartFragment}`;
var CART_BUYER_IDENTITY_UPDATE_MUTATION = `mutation cartBuyerIdentityUpdate($cartId: ID!, $buyerIdentity: CartBuyerIdentityInput!) { cartBuyerIdentityUpdate(cartId: $cartId, buyerIdentity: $buyerIdentity) { cart { ...CartFragment } userErrors { field message } } } ${CartFragment}`;

// js/cart-core.js
var cart = null;
var isLoading = false;
var elevarCartInitialized = false;
var UVLIZER_PREFIX = "uvlizer_";
var CART_ID_STORAGE_KEY = `${UVLIZER_PREFIX}cart_id`;
var cartLoadingOverlay = document.getElementById("cart-loading-overlay");
var cartErrorContainer = document.getElementById("cart-error");
var cartErrorMessage = document.getElementById("cart-error-message");
...
async function callShopifyAPI(query, variables = {}) {
  ...
  try {
    ...
    if (cartErrorMessage && cartErrorContainer) {
      ...
      alert(error.message || "An API error occurred.");
    }
    throw error;
  } finally {
    if (cartLoadingOverlay && checkoutBtn) {
...
function createElevarProductObjCore(item) {
  ...
  return {
    ...
    // Minimal fields needed for dl_user_data
  };
}
function createElevarViewItemObj() {
  return {
    ...
    price: "199.00",
    // Use default or fetch if critical
    product_id: "8000627572985",
    variant_id: "44103628357881",
    // Default variant ID
  ...
  document.addEventListener("alia:signup", async (e) => {
    ...
    if (!cart) {
      try {
        ...
        console.error("Cart needed for signup but failed to init:", err);
        return;
      }
    }
    if (!cart) return;
  ...
  document.addEventListener("alia:pollAnswered", async (e) => {
    ...
    if (!cart) {
      try {
        ...
        console.error("Cart needed for poll but failed to init:", err);
        return;
      }
    }
    if (!cart) return;
    ...
    try {
      ...
      if (data?.cartAttributesUpdate?.cart) {
        cart = data.cartAttributesUpdate.cart;
      } else {
...
var getCart = () => cartPromise;
var cartReady = cartPromise.then(() => {
});
...
var cartItemsContainer = cartDrawer.querySelector(".cart-items-container");
...
var checkoutBtn2 = document.getElementById("checkout-btn");
var cartLoadingOverlay2 = document.getElementById("cart-loading-overlay");
var cartErrorContainer2 = document.getElementById("cart-error");
var cartErrorMessage2 = document.getElementById("cart-error-message");
var aliaDiscountSection = document.getElementById("alia-discount-section");
var aliaRewardTextElement = document.getElementById("alia-reward-text");
var aliaDiscountCodeElement = document.getElementById("alia-discount-code");
var aliaTimerElement = document.getElementById("alia-timer");
var aliaExpiredMessageElement = document.getElementById("alia-expired-message");
var cart2 = null;
var aliaTimerInterval = null;
var isLoading2 = false;
var previousActiveElement = null;
var UVLIZER_PREFIX2 = "uvlizer_";
var ALIA_CODE_STORAGE_KEY = `${UVLIZER_PREFIX2}alia_discount_code`;
var ALIA_TEXT_STORAGE_KEY = `${UVLIZER_PREFIX2}alia_reward_text`;
var ALIA_EXPIRY_STORAGE_KEY = `${UVLIZER_PREFIX2}alia_expiry_timestamp`;
function setLoading(state) {
  isLoading2 = state;
  if (cartLoadingOverlay2) cartLoadingOverlay2.classList.toggle("hidden", !state);
  document.body.style.cursor = state ? "wait" : "default";
  if (checkoutBtn2) {
    checkoutBtn2.disabled = state || cart2 && cart2.totalQuantity === 0;
    checkoutBtn2.classList.toggle("opacity-50", state || cart2 && cart2.totalQuantity === 0);
  }
  cartItemsContainer?.querySelectorAll(".remove-item-btn, .quantity-btn").forEach((btn) => btn.disabled = state);
}
function showCartError(message, autoHideDelay = 5e3) {
  if (window.cartErrorTimeout) clearTimeout(window.cartErrorTimeout);
  if (cartErrorMessage2 && cartErrorContainer2) {
    cartErrorMessage2.textContent = message;
    cartErrorContainer2.classList.remove("hidden");
    setTimeout(() => {
      cartErrorContainer2.classList.add("visible");
      if (typeof lucide !== "undefined") lucide.createIcons({ nodes: cartErrorContainer2.querySelectorAll("[data-lucide]") });
    }, 10);
    if (autoHideDelay > 0) {
...
function hideCartError() {
  if (cartErrorContainer2) {
    cartErrorContainer2.classList.remove("visible");
    setTimeout(() => cartErrorContainer2.classList.add("hidden"), 300);
  }
}
...
async function removeCartLineDrawer(cartId, lineId) {
  ...
  if (cart2 && window.ElevarDataLayer) {
    const lineToRemove = cart2.lines.nodes.find((line) => line.id === lineId);
    if (lineToRemove) {
  ...
  if (updatedCart) cart2 = updatedCart;
  return updatedCart;
}
...
function renderCartItems() {
  ...
  if (!cart2 || !cart2.lines?.nodes.length || !template || !cartItemsContainer) {
    if (emptyCartMessage) {
      ...
      if (continueShoppingBtnEmpty && !continueShoppingBtnEmpty.getAttribute("data-listener-added")) {
        continueShoppingBtnEmpty.addEventListener("click", (e) => {
          e.preventDefault();
          closeDrawer();
        });
  ...
  const currentLineIds = new Set(cart2.lines.nodes.map((item) => item.id));
  ...
  cart2.lines.nodes.forEach((item) => {
...
function updateCartIconCount() {
  ...
  const total = cart2?.totalQuantity || 0;
...
function renderCartSummary() {
  ...
  if (checkoutBtn2) {
    checkoutBtn2.disabled = finalPrice === 0;
    checkoutBtn2.classList.toggle("opacity-50", finalPrice === 0);
    checkoutBtn2.setAttribute("data-checkout-url", cart2.checkoutUrl || "#");
  }
...
function renderAppliedDiscounts() {
  ...
  const isCodeInShopifyCart = cart2?.discountCodes?.some((dc) => dc.code === aliaCode);
  if (isLocalStorageValid && isCodeInShopifyCart) {
    ...
    startAliaTimer(Math.floor((storedExpiry - Date.now()) / 1e3));
  } else {
...
function startAliaTimer(durationSeconds) {
  ...
  aliaTimerInterval = setInterval(() => {
    ...
    if (timer < 0) {
      ...
      if (cart2?.id) {
        applyDiscountCode(cart2.id).then((updatedCart) => {
          if (updatedCart) {
            cart2 = updatedCart;
            renderCart();
...
function openDrawer() {
  ...
  if (cart2) {
    renderCart();
    if (window.ElevarDataLayer && cart2.lines?.nodes.length > 0) {
      const impressions = cart2.lines.nodes.map((item, index) => createElevarImpressionObj(item, index));
      let cartTotalRaw = cart2?.cost?.totalAmount?.amount;
      let cartTotalInCents = 0;
      try {
        if (typeof cartTotalRaw === "number") cartTotalInCents = Number.isInteger(cartTotalRaw) ? cartTotalRaw : Math.round(cartTotalRaw * 100);
        else if (typeof cartTotalRaw === "string") {
      ...
      const viewCartEvent = {
        ...
        cart_total: (cartTotalInCents / 100).toFixed(2),
        ecommerce: {
          currencyCode: cart2.cost?.totalAmount?.currencyCode || "USD",
          actionField: { list: location.pathname },
          impressions
        }
      };
...
async function handleRemoveItem(eventOrButton) {
  ...
  if (!cart2 || !lineId) return;
  try {
    const updatedCart = await removeCartLineDrawer(cart2.id, lineId);
    if (updatedCart) {
      renderCart();
    }
  } catch (error) {
...
async function handleUpdateItemQuantity(lineId, newQuantity) {
  if (!cart2 || !lineId || newQuantity < 0) return;
  try {
    let updatedCart;
    if (newQuantity === 0) {
      updatedCart = await removeCartLineDrawer(cart2.id, lineId);
    } else {
...
async function handleAddItemClick(variantId) {
  if (!variantId || !cart2?.id) return;
  try {
    const updatedCart = await addCartLine(cart2.id, variantId, 1);
    if (updatedCart) {
      cart2 = updatedCart;
      ...
      if (addedLine) {
        ...
        const klaviyoCartData = {
          ...
          original_total_price: parseFloat(updatedCart.cost.subtotalAmount.amount),
          items: klaviyoCartItems
        };
        var _learnq = window._learnq || [];
        _learnq.push(["track", "Added to Cart", klaviyoCartData]);
        const cartBeforeAdd = JSON.parse(JSON.stringify(cart2));
        ...
        productForEvent.quantity = quantityAdded > 0 ? quantityAdded.toString() : "1";
...
function setupEventListeners() {
  ...
  if (checkoutBtn2) {
    checkoutBtn2.addEventListener("click", () => {
      const baseUrl = cart2?.checkoutUrl;
      if (window.checkoutResetTimeout) clearTimeout(window.checkoutResetTimeout);
      if (cart2 && cart2.totalQuantity > 0 && baseUrl && baseUrl !== "#") {
        checkoutBtn2.disabled = true;
        checkoutBtn2.classList.add("opacity-50");
        checkoutBtn2.textContent = "Redirecting...";
        const params = new URLSearchParams();
        const queryString = params.toString();
        let finalUrl = baseUrl + (queryString ? (baseUrl.includes("?") ? "&" : "?") + queryString : "");
        window.location.href = finalUrl;
        window.checkoutResetTimeout = setTimeout(() => {
          checkoutBtn2.disabled = false;
          checkoutBtn2.classList.remove("opacity-50");
          checkoutBtn2.textContent = "Proceed to Checkout";
          window.checkoutResetTimeout = null;
        }, 5e3);
      } else {
        showCartError(cart2?.totalQuantity === 0 ? "Your cart is empty." : "Could not proceed to checkout.");
      }
    });
  }
  document.querySelectorAll(".js-main-cta").forEach((button) => {
    button.addEventListener("click", (e) => {
      ...
      const isProductInCart = cart2?.lines?.nodes?.some((line) => line.merchandise.id === selectedVariantId);
      if (isProductInCart) openDrawer();
      else handleAddItemClick(selectedVariantId);
    });
  });
  if (cartItemsContainer) {
    cartItemsContainer.addEventListener("click", function(event2) {
      const minusBtn = event2.target.closest(".quantity-minus-btn");
      if (minusBtn) {
      ...
      const plusBtn = event2.target.closest(".quantity-plus-btn");
      if (plusBtn) {
      ...
      const removeBtn = event2.target.closest(".remove-item-btn");
      if (removeBtn) {
  ...
  document.addEventListener("alia:rewardClaimed", async (e) => {
    ...
    if (!discountCode || !cart2?.id) return;
    const expiryTimestamp = Date.now() + 30 * 60 * 1e3;
    try {
      const updatedCart = await applyDiscountCode(cart2.id, discountCode);
      if (updatedCart) {
...
function handleFocusTrap(event2) {
  if (event2.key !== "Tab" || !cartDrawer) return;
  const focusableElements = cartDrawer.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
...
var lucide2 = (() => {
  ...
  var D = (e, a) => {
    for (var r in a) c(e, r, { get: a[r], enumerable: true });
  }, L = (e, a, r, t) => {
    if (a && typeof a == "object" || typeof a == "function") for (let o of M(a)) !F.call(e, o) && o !== r && c(e, o, { get: () => a[o], enumerable: !(t = B(a, o)) || t.enumerable });
    return e;
  };
  var R = (e) => L(c({}, "__esModule", { value: true }), e);
  var U = {};
  D(U, { AlertCircle: () => f, Heart: () => l, Minus: () => s, Plus: () => u, ShoppingCart: () => d, User: () => p, X: () => m, createIcons: () => n });
  ...
  var P = ([e, a, r]) => {
    let t = document.createElementNS("http://www.w3.org/2000/svg", e);
    return Object.keys(a).forEach((o) => {
      t.setAttribute(o, String(a[o]));
  ...
  var y = (e) => Array.from(e.attributes).reduce((a, r) => (a[r.name] = r.value, a), {}), q = (e) => typeof e == "string" ? e : !e || !e.class ? "" : e.class && typeof e.class == "string" ? e.class.split(" ") : e.class && Array.isArray(e.class) ? e.class : "", T = (e) => e.flatMap(q).map((r) => r.trim()).filter(Boolean).filter((r, t, o) => o.indexOf(r) === t).join(" "), b = (e) => e.replace(/(\w)(\w*)(_|-|\s*)/g, (a, r, t) => r.toUpperCase() + t.toLowerCase()), C = (e, { nameAttr: a, icons: r, attrs: t }) => {
    let o = e.getAttribute(a);
    if (o == null) return;
    let x = b(o), h = r[x];
    if (!h) return console.warn(`${e.outerHTML} icon name was not found in the provided icons object.`);
    let S = y(e), g = { ...i, "data-lucide": o, ...t, ...S }, w = T(["lucide", `lucide-${o}`, S, t]);
    w && Object.assign(g, { class: w });
    let A = k(h, g);
  ...
  var l = [["path", { d: "M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z" }]];
  var s = [["path", { d: "M5 12h14" }]];
  var u = [["path", { d: "M5 12h14" }], ["path", { d: "M12 5v14" }]];
  var d = [["circle", { cx: "8", cy: "21", r: "1" }], ["circle", { cx: "19", cy: "21", r: "1" }], ["path", { d: "M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12" }]];
  var p = [["path", { d: "M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" }], ["circle", { cx: "12", cy: "7", r: "4" }]];
  var m = [["path", { d: "M18 6 6 18" }], ["path", { d: "m6 6 12 12" }]];
  var n = ({ icons: e = {}, nameAttr: a = "data-lucide", attrs: r = {} } = {}) => {
...
lucide.createIcons({icons});\``);
    ...
    if (Array.from(t).forEach((o) => C(o, { nameAttr: a, icons: e, attrs: r })), a === "data-lucide") {
      let o = document.querySelectorAll("[icon-name]");
      o.length > 0 && (console.warn("[Lucide] Some icons were found with the now deprecated icon-name attribute. These will still be replaced for backwards compatibility, but will no longer be supported in v1.0 and you should switch to data-lucide"), Array.from(o).forEach((x) => C(x, { nameAttr: "icon-name", icons: e, attrs: r })));
    }
  };
  window.addEventListener("DOMContentLoaded", () => {
    n({ icons: { X: m, Plus: u, Minus: s, ShoppingCart: d, Heart: l, User: p, AlertCircle: f } });
  });
  window.lucide = { createIcons: n, X: m, Plus: u, Minus: s, ShoppingCart: d, Heart: l, User: p, AlertCircle: f, icons: { X: m, Plus: u, Minus: s, ShoppingCart: d, Heart: l, User: p, AlertCircle: f } };
  return R(U);
})();
